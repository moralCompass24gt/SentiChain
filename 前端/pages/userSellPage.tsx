...
const userSellRecord= () => {
  ...
  useEffect(() => {
    ...
    const fetchRecords = async () => {
      try {
        const response = await fetch(`http://81.71.5.116:9090/usersgoodsdetail?address=${address}`, {
          method: "GET",
          headers: {
                ...
                record.IsSold?(
                  ...
                  <div className="card bg-base-100 shadow-xl text-black m-5 glass">
                    <div className="card-body">
                      ...
                      >
                        ...
                        <p className="text-left break-all">交易价格：{record.ExpectPrice}元</p>
                        <p className="text-left break-all text-xl">已出售</p>
                      </div>
                      <p className="text-3xl m-5 text-left break-all">"{record.StoryDetail}"</p>
                      <p className="text-left">商品出售时间：{record.Time}</p>
                    </div>
                  </div>
                </Link>
                ):(
                  ...
                  <div className="card bg-base-100 shadow-xl text-black m-5 glass">
                    <div className="card-body">
                      ...
                      >
                        <h2 className="card-title text-left">{record.CommodityName}</h2>
                        <p className="text-left break-all">预期价格：{record.ExpectPrice}元</p>
                        <p className="text-left break-all text-xl">已发布</p>
                      </div>
                      <p className="text-3xl m-5 text-left break-all">"{record.StoryDetail}"</p>
                      <p className="text-left">商品发布时间：{record.Time}</p>
                    </div>
                  </div>
                </Link>
                )
